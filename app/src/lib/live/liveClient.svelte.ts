...
export function connectLive(port: number = DEFAULT_PORT): void {
	...
	ws.onmessage = (ev) => {
		...
		if (msg.type === "hello") {
			...
			// Safety: every new live attach starts ARMED - folding is
			// on by default per this fork, as the user prefers.
			folding.enabled = true;
			// Sync the armed state to the extension on every attach, so a fresh
			// (or reconnected) extension learns this client's armed state over the wire
			// from turn zero rather than inferring it.
			sendArmed(true);
